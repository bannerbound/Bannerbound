package me.bannerbound.com.pms.civpm.entities;
...
public class CPMFakeCitizenEntity extends Mob {
    ...
    private void simplifiedWalkingTick() {
        if (walkTargetVec == null) return;
        ...
        if (distanceSqr <= (flySpeed * flySpeed) || distanceSqr < 0.25) {
            this.setPos(walkTargetVec);
            this.walkTargetVec = null;
            this.walkTimeoutSpent = 0;
            this.setDeltaMovement(Vec3.ZERO);
        } else {
            Vec3 step = dir.normalize().scale(flySpeed);
            this.setPos(pos.add(step));
            this.setDeltaMovement(step);

            float targetAngle = (float) (Mth.atan2(-dir.x, dir.z) * (180F / (float) Math.PI));

            float smoothBodyAngle = Mth.approachDegrees(this.yBodyRot, targetAngle, 15.0F);
            this.setYRot(smoothBodyAngle);
            this.setYBodyRot(smoothBodyAngle);
            this.setYHeadRot(smoothBodyAngle);
        }
    }
    ...
    public void tick() {
        ...
        if (!BannerboundClientConfig.cpmSimplifiedWalking) {
            this.physicsTick();
        }
